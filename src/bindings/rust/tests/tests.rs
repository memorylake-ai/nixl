...
mod mem_view {
    ...
    /// Allocates one VRAM buffer, registers it, and returns it with a matching
    /// single-descriptor list. `None` when no GPU is available.
    ...
    fn vram_remote_dlist<'a>(descriptor: &'a RemoteDescriptor<'a>) -> RemoteDescList<'a> {
    ...
    ///     NIXL_TEST_DEVICE_LANE=1 cargo test --test tests mem_view
    #[test]
